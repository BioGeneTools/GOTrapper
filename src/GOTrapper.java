...
public class GOTrapper {
	...
	public boolean radioButtonGeneSym = true;
	...
	public static JRadioButton rdbtnGeneSym;
	public static JRadioButton rdbtnEntrezID;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					// 
					String[] FileMustExist = {"GOidvsGeneSym.tsv","GOidvsGeneID.tsv","GObpALLvsid.tsv","GOmfALLvsid.tsv","goterm.tsv",
							"gobp.tsv","gomf.tsv","bpGOvsNumgene.tsv","mfGOvsNumgene.tsv"};
					window.CheckNecessaryFile(FileMustExist);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() throws FileNotFoundException {
		...
		spinner.setBounds(357, 102, 65, 28);
		...
		Scroll_textArea_genelist1.setBounds(6, 6, 114, 231);
		...
		Scroll_textArea_genelist2.setBounds(122, 6, 114, 231);
		...
		//Start Radio Buttons
		rdbtnGeneSym = new JRadioButton("Gene Sym");
		rdbtnGeneSym.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(btnGO_pressed==false){
					rdbtnGeneSym.setSelected(true);
					radioButtonGeneSym = true;
				}else{ 
					if(radioButtonGeneSym==false){
						rdbtnEntrezID.setSelected(true);
					}
				}
			}
		});
		rdbtnGeneSym.setSelected(true);
		rdbtnGeneSym.setBounds(274, 50, 93, 23);
		frame.getContentPane().add(rdbtnGeneSym);
		
		rdbtnEntrezID = new JRadioButton("Gene ID");
		rdbtnEntrezID.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(btnGO_pressed==false){
					rdbtnEntrezID.setSelected(true);
					radioButtonGeneSym = false;
		...
		rdbtnEntrezID.setSelected(false);
		rdbtnEntrezID.setBounds(367, 50, 93, 23);
		frame.getContentPane().add(rdbtnEntrezID);
		...
		rdbtnWithin.setSelected(true);
		rdbtnWithin.setBounds(274, 75, 75, 23);
		...
		rdbtnBetween.setBounds(367, 75, 84, 23);
		...
		group2.add(rdbtnGeneSym);
		group2.add(rdbtnEntrezID);
		//END Radio Buttons
		
		//Start JTable
		scrollPane = new JScrollPane();
		scrollPane.setBounds(20, 311, 487, 177);
      ...
      //END JTable
		...
		btnExport.setBounds(30, 497, 117, 29);
		...
		btnStatistics.setBounds(145, 497, 117, 29);
		...
		btnInfo.setBounds(260, 497, 117, 29);
		...
		panel_1.setBounds(274, 132, 233, 164);
		...
		lblThreshold.setBounds(284, 107, 84, 16);
	...
	public void runGO() throws FileNotFoundException{
		...
		CallIDs("GOidvsGeneID.tsv","J");
		
		CallIDs("GObpALLvsid.tsv","E");
		CallIDs("GOmfALLvsid.tsv","F");

		IDsgenes.getSymID(IDsGOidSym.idx,IDsGOidSym.idy, IDsGOidEntrezID.idx, IDsGOidEntrezID.idy, radioButtonGeneSym);
	...
	public void CallIDs(String x, String y) throws FileNotFoundException{
		if(y=="A"){ // from TextArea into idx and idy array in IDsgenes object
			IDsgenes = new IDs();
			...
			IDsGOidSym.getIDs(x);
		}else{ // "J" Gene Entrez ID
			IDsGOidEntrezID = new GObpmf();
			IDsGOidEntrezID.getIDs(x);
		}
	}
	...
	public void CheckNecessaryFile(String[] x){
		...
		if(!nonExist.isEmpty()){
			JOptionPane.showMessageDialog(null, "The following required file(s) are not exist. \n"
					+ "Please make sure they are in the program directory. \nFiles: "+nonExist.toString().replace("[", "").replace("]", ""));
		}
