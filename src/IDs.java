...
public class IDs{
	...
	public void getSymID(ArrayList<String> a, ArrayList<String> b){
		for (String x:idx){
			for (String y:b){
				if (x.equals(y)){
	...
	public void RefinedSym(ArrayList<String> a, ArrayList<String> b, ArrayList<String> aa, ArrayList<String> bb, ArrayList<String> GO_id, ArrayList<String> GO_term, ArrayList<String> GO_cat){
		if(GOTrapper.rdbtnWithin.isSelected()==true){
			...
			for (int i=0; i<=Refined.size(); i++){
				GOTrapper.model.addRow(new Object[]{i+1, Refined.get(i), Refined_GOcat.get(i), Refined_BackgroundGene.get(i), Refinedsym.get(i), Refined_Score.get(i), Refined_GOterm.get(i)});
			}
		}else{
			for (String x:Refined){ 
				ArrayList<String> Sym = new ArrayList<String>();
			...
			for (int i=0; i<=Refined.size(); i++){
				GOTrapper.model.addRow(new Object[]{i+1, Refined.get(i), Refined_GOcat.get(i), Refined_BackgroundGene.get(i), Refinedsym.get(i).replace("[", "").replace("]", ""), Refined_Score.get(i), Refined_GOterm.get(i)});
			}
		}
	}
	...
	public void ExportToFile(File FilePath) throws FileNotFoundException{
		PrintWriter pw = new PrintWriter(new File(FilePath+File.separator+"GOtoolResult.csv"));
	...
	public void statistics(){
		if(GOTrapper.rdbtnWithin.isSelected()==true){
			JOptionPane.showMessageDialog(null, "# of Genes not exist: "+(idx.size()-id_genelist1.size())+"\n# of Genes without GO Term: "+
					(id_genelist1Sym.size()-idGO_genelist1.size())+"\n# of Shared GO Terms: "+idGOshared.size()+
					"\n# of Refined Shared GO Terms: "+Refinedid.size());
		}else{
			JOptionPane.showMessageDialog(null, "# of Genes not exist: "+((idx.size()+idy.size())-(id_genelist1.size()+id_genelist2.size()))+"\n# of Genes without GO Term: "+
